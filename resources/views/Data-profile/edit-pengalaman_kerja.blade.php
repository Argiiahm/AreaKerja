...
@section('content')
    <section class="pt-40 pb-20 bg-gray-50/50">
        ...
        <h1 class="font-bold text-2xl mx-4 sm:mx-10 text-gray-800">Edit Pengalaman Kerja</h1>
            ...
            <div class="flex flex-col lg:flex-row lg:items-center gap-8 lg:gap-14">
                ...
                <div class="flex flex-col w-full">
                    <div class="mb-6 text-center lg:text-left">
                        ...
                        <p class="text-gray-500 text-sm">Perbarui detail pengalaman profesional Anda untuk memikat rekruter.</p>
                    </div>
        ...
        <div class="mt-10 mx-4 sm:mx-10 bg-white p-8 sm:p-12 rounded-3xl border border-gray-100 shadow-sm">
            <h2 class="text-xl font-bold text-gray-800 border-b-2 border-orange-500 pb-2 mb-8 flex items-center gap-2">
                <i class="ph ph-buildings text-orange-500 text-2xl"></i>
                Detail Pengalaman Kerja
            </h2>

            <form class="grid grid-cols-1 md:grid-cols-2 gap-6" action="/update/pengalaman/kerja/{{ $data->id }}" method="POST">
                @csrf
                @method('PUT')
                
                <div>
                    <label for="posisi_kerja" class="block mb-2 text-sm font-semibold text-gray-700">Posisi Kerja</label>
                    <input type="text" name="posisi_kerja" id="posisi_kerja"
                        ...
                        placeholder="Contoh: Senior Web Developer" value="{{ $data->posisi_kerja }}" />
                </div>

                <div>
                    <label for="jabatan_kerja" class="block mb-2 text-sm font-semibold text-gray-700">Jabatan Kerja</label>
                    <input type="text" name="jabatan_kerja" id="jabatan_kerja"
                        class="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-orange-400 focus:outline-none transition-all"
                        placeholder="Contoh: Manager / Supervisor" value="{{ $data->jabatan_kerja }}" />
                </div>

                <div class="md:col-span-2">
                    <label for="nama_perusahaan" class="block mb-2 text-sm font-semibold text-gray-700">Nama Perusahaan</label>
                    <div class="relative">
                        ...
                        <i class="ph ph-office-building absolute left-3 top-3.5 text-gray-400 text-lg"></i>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="tahun_awal" id="tahun_awal"
                            class="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-orange-400 focus:outline-none transition-all"
                            placeholder="2019" value="{{ $data->tahun_awal }}" />
                    </div>
                    <div>
                        <label for="tahun_akhir" class="block mb-2 text-sm font-semibold text-gray-700">Tahun Selesai</label>
                        <input type="text" name="tahun_akhir" id="tahun_akhir"
                            class="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-orange-400 focus:outline-none transition-all"
                            placeholder="2023 / Sekarang" value="{{ $data->tahun_akhir }}" />
                    </div>
                </div>

                <div class="md:col-span-2">
                    <label for="deskripsi" class="block mb-2 text-sm font-semibold text-gray-700">Deskripsi Pekerjaan</label>
                    <textarea name="deskripsi" id="deskripsi" rows="5"
                        class="w-full border border-gray-300 rounded-xl p-3 focus:ring-2 focus:ring-orange-400 focus:outline-none transition-all"
                        placeholder="Jelaskan tanggung jawab dan pencapaian Anda selama bekerja...">{{ $data->deskripsi }}</textarea>
                </div>
                ...
                <div class="md:col-span-2 pt-4">
                    <button type="submit"
                        ...
                        <i class="ph ph-floppy-disk-back text-xl group-hover:rotate-12 transition-transform"></i>
