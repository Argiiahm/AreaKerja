@extends('layouts.index')
@section('content')
<div class="w-full">
    <div class="max-w-3xl mx-auto px-8 py-12 mt-40 bg-white rounded-xl shadow-md border border-gray-300">

        <div class="flex items-center gap-6 mb-10">
            <img src="{{ asset('Icon/seveninc.png') }}" alt="Logo" class="w-48 h-auto">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Selamat Kepada</h2>
                <p class="mt-4 text-gray-800">
                    <span class="block font-semibold text-lg">Seno Robloxx </span>
                    <span class="block text-sm">Status : Belum Kerja</span>
                </p> 
            </div>
        </div>

        <div class="text-gray-800 font-serif text-[15px] leading-relaxed space-y-5 max-w-2xl mx-auto text-justify">
            <p>
                Lamaran yang anda ajukan ke lowongan kami pada <span class="font-semibold">Divisi Videografi</span> 
                telah kami <span class="text-green-600 font-bold">Terima</span>.
            </p>

            <p>
                Oleh karena itu, dengan hormat kami mengundang anda untuk hadir pada:
            </p>

            <div class="pl-6 space-y-2">
                <p><span class="font-semibold">Tanggal</span> : 08 Februari 2023</p>
                <p><span class="font-semibold">Pukul</span> : 09 : 00</p>
                <p><span class="font-semibold">Tempat</span> : Kantor Seven Inc</p>
                <p><span class="font-semibold">Keperluan</span> : Wawancara Kerja</p>
            </div>

            <p>
                <span class="font-semibold">Catatan</span> : Berpakaian sopan dan rapi dengan kemeja putih 
                serta celana bahan hitam. Harap membawa CV, KTP, Ijazah SMA/SMK, Transkrip Nilai Kuliah, 
                serta dokumen-dokumen pendukung lainnya (jika ada).
            </p>

            <div class="pt-6">
                <p>Hormat kami,</p>
                <p class="mt-1 font-semibold">Seven Inc</p>
            </div>
        </div>

        <div class="flex flex-col items-center mt-16">
            <img src="{{ asset('image/logo-areakerja.png') }}" alt="Logo Footer" class="h-10 mb-3">
            <p class="text-center text-xs text-gray-500">Copyright ©2024 areakerja.com</p>
        </div>
    </div>

    <div class="flex justify-end gap-4 mt-8 max-w-3xl mx-auto px-8">
        <button class="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2 rounded-md shadow font-medium">
            Kembali
        </button>
        <button class="bg-orange-500 hover:bg-orange-600 text-white px-6 py-2 rounded-md shadow font-medium">
            Kirim
        </button>
    </div>
</div>
@endsection
