...
<div class="mx-aut p-6 mt-8 border-2 border-gray-200 rounded-lg px-5 lg:px-20 md:px-5">
    ...
    <form action="" method="POST" class="space-y-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="email" name="email" value="{{ Auth::user()->email }}"
                    class="w-full border-2 border-gray-200 rounded-md p-2" disabled>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
                ...
                <select name="provinsi" class="w-full border border-gray-200 rounded-md p-2">
                    <option selected>{{ Auth::user()->admin->provinsi ?? 'Data Belum Dilengkapi' }}</option>
                </select>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Kota</label>
                <select name="kota" class="w-full border border-gray-200 rounded-md p-2">
                    <option selected>{{ Auth::user()->admin->kota ?? 'Data Belum Dilengkapi' }}</option>
                </select>
            </div>

            <div>
                <label class="block text-sm font-medium mb-1">Kecamatan</label>
                <select name="kecamatan" class="w-full border border-gray-200 rounded-md p-2">
                    <option selected>{{ Auth::user()->admin->kecamatan ?? 'Data Belum Dilengkapi' }}</option>
                </select>
            </div>
        </div>
