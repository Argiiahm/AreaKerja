                    ...
                    <tbody class="text-center">
                        ...
                        <tr class="px-6 py-4 whitespace-no-wrap border-gray-500 border-b">
                                ...
                                <td class="py-4 whitespace-no-wrap border-gray-500 text-blue-500">
                                    <a href="/dashboard/admin/event/detail/{{ $d->id }}">{{ $d->title }}</a>
                                </td>
                                <td class=" py-4 whitespace-no-wrap  border-gray-500 text-sm leading-5">{{ $d->kuota }}</td>
                                <td class=" py-4 whitespace-no-wrap  border-gray-500 text-sm leading-5">{{ $d->tgl_mulai }}<span class="pl-2 text-gray-600">{{ $d->jam_mulai }}</span></td>
                                <td class=" py-4 whitespace-no-wrap  border-gray-500 text-sm leading-5">{{ $d->tgl_akhir }}<span class="pl-2 text-gray-600">{{ $d->jam_akhir }}</span></td>
                                <td
                                    class=" py-4 whitespace-no-wrap text-white font-bold  border-gray-500 text-sm leading-5">
                                    <form action="/dashboard/admin/event/hapus/{{ $d->id }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" onclick="return confirm('Apakah Anda Yakin')"><i class="ph ph-trash text-2xl bg-gray-700 p-0.5"></i></button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
